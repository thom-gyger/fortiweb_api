...
class DdosHttpAccessLimit(EndpointBase):
    q_ref: Optional[int]
    q_ref_string: Optional[str]
    name: str = field(metadata=EndpointBase.post_mark | EndpointBase.key_mark)
    access_limit_standalone_ip: int = field(metadata=EndpointBase.post_mark)
    access_limit_share_ip: int = field(metadata=EndpointBase.post_mark)
